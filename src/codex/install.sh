...
# From: https://github.com/jsburckhardt/devcontainer-features/blob/main/src/codex/install.sh
# (c) jsburckhardt, licensed under MIT License.
# (c) Niko Föhr (modifications), licensed under MIT License.
...
if [ -n "${_REMOTE_USER:-}" ]; then
    ...
    chown "$_REMOTE_USER:$_REMOTE_USER" /codex-config
fi

# Add the post-install script for persistent storage for Codex configuration
install -d /usr/local/bin
install -m 0755 ./link-codex-config.sh /usr/local/bin/link-codex-config
